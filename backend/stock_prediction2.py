...
Be realistic, concise, and return ONLY the JSON."""
    ...
    async def _generate_predictions_with_ai(self, prompt: str) -> Dict[str, Any]:
        ...
        """
        # Use OpenAI ChatCompletion (gpt-4). Read API key from OPENAI_API_KEY if present.
        ...
        # Normalize to a content string for parsing
        response = resp
        ...
        # OpenAI-style response objects often have .choices[0].message.content or .choices[0].text
        try:
            if hasattr(response, 'choices'):
                ...
                if choices and len(choices) > 0:
                    ...
                    if not msg:
                        msg = getattr(ch, 'text', None) or getattr(ch, 'content', None)
    ...
    async def predict_article_impact(self, assets: List[str], article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Generate AI predictions using OpenAI (async call)
        ...
        # For each predicted asset, build a combined weekly series (historical weeks + predicted weeks)
        for symbol, pred_data in filtered_predictions.items():
            ...
            predicted_points = []
            for week_num, price in enumerate(future_prices, start=1):
                try:
                    price_val = float(price) if price is not None else None
                except (TypeError, ValueError):
                    price_val = None

                future_date = last_hist_date + timedelta(weeks=week_num)
